...
VBObox2.prototype.draw = function() {
    if (runMode > 1){
        if (runMode == 2) runMode = 1;  // do one step
         ...
         this.pSys.dotfinder(this.pSys.S0dot, this.pSys.S0, g_timeStep);
...
function VBObox3(){
    ...
    this.pSys = new CPartSys();
    var row = 10, col = 10;
    this.partCount = row * col;
    this.forces = [F_NONE, F_DRAG,F_GRAV_E,F_SPRINGNET];
    this.walls = [WTYPE_GROUND];//WTYPE_PBALL, WTYPE_GROUND, WTYPE_XWALL_LO, WTYPE_XWALL_HI, WTYPE_YWALL_LO, WTYPE_YWALL_HI,  WTYPE_ZWALL_LO, WTYPE_ZWALL_HI];
    this.pSys.initSpringNet(row, col, this.forces, this.walls);
...
function VBObox5(){
    ...
    this.pSys = new CPartSys();
    this.partCount = 600;
    this.forces = [F_NONE, F_GRAV_E, F_DRAG,F_FIRE];
    this.walls = [FIRE_CONSTRAINT, WTYPE_GROUND, WTYPE_PBALL, WTYPE_XWALL_LO, WTYPE_XWALL_HI, WTYPE_YWALL_LO, WTYPE_YWALL_HI,  WTYPE_ZWALL_LO, WTYPE_ZWALL_HI];
    this.pSys.initBouncyBall(this.partCount, this.forces, this.walls);
